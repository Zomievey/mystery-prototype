    ...
    useEffect(() => {
        ...
        if (raw) {
            try {
                ...
                if (!parsed.id) parsed.id = `tmp-${Math.random().toString(36).slice(2, 10)}`;
                setRecipe(parsed);
            } catch {
                setRecipe(null);
            }
        ...
        <main className="mx-auto max-w-3xl px-4 py-10">
            <div className="rounded-xl border border-border bg-card p-6 space-y-6">
                <div className="flex items-center justify-between">
                    ...
                    <Link href="/recipes" className="text-sm text-foreground/70 hover:underline">
                        Back to recipes
                    </Link>
                </div>
                ...
                <div className="grid gap-8 md:grid-cols-2">
                        ...
                        {recipe.instructions?.length ? (
                            <ol className="list-decimal pl-5 space-y-2">
                                {recipe.instructions.map((step, i) => (
                                    <li key={`${recipe.id}-step-${i}`} className="text-sm leading-relaxed">
                                        {step}
                                    </li>
                                ))}
                            </ol>
                        ) : (
                            <p className="text-sm text-foreground/60">No instructions detected.</p>
                        )}
                    </section>
                    ...
                    <section>
                        ...
                        {recipe.ingredients?.length ? (
                            ...
                            <p className="text-sm text-foreground/60">No ingredients detected.</p>
                        )}
                    </section>
                </div>
