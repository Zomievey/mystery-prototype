...
export async function parseRecipeFromText(
    ...
    if (!inIngredientsSection && !inInstructionsSection) {
        const { ingredients, instructions } =
            separateIngredientsAndInstructions(lines, normalizeQuotes, isLikelyInstruction);
...
function separateIngredientsAndInstructions(
    ...
    for (let line of lines) {
        if (
            !line.trim() ||
            line.toLowerCase().match(/^(ingredients|instructions|directions|method):?$/i)
        ) {
            continue;
        }
...
function parseIngredientLine(line: string): Ingredient | null {
    ...
    for (const pattern of patterns) {
        ...
        if (match?.groups) {
            const { qty = "", unit = "", name = "" } = match.groups as {
                qty?: string;
                unit?: string;
                name?: string;
            };
                    ...
                    m.status === "loading language traineddata"
                        ...
                        : m.status === "initialized api"
                            ...
                            : m.status === "recognizing text"
                                ...
                                : m.status ?? "Working...";
                const bar = m.status === "recognizing text" ? 10 + pct * 0.8 : m.status ? 10 : 0;
